...
fn validate_config(raw: &str) -> Result<(), String> {
    ...
    if config.product_name.as_deref() != Some("Subtitle Renamer")
        ...
        || config.version.as_deref() != Some(env!("CARGO_PKG_VERSION"))
        || config.identifier != "de.lidco.subtitlerenamer"
    {
        return Err("desktop identity changed".into());
    }
    if config.build.frontend_dist != Some(FrontendDist::Directory(PathBuf::from("../../dist")))
        || config.build.dev_url.as_ref().map(|u| u.as_str()) != Some("http://localhost:1420/")
    ...
    {
        return Err("react/vite frontend contract changed".into());
    }
...
fn configuration_guard_rejects_forbidden_policy_changes() {
    ...
    for case in &cases {
        assert_ne!(case.as_str(), CONFIG, "mutation produced no change");
