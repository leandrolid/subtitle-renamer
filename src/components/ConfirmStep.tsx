...
import type { CopyRow, Locale, Outcome, Phase, VisibleStep } from "../types.ts";
      ...
      {showEmptyForFailure && (
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <h3 className="text-sm font-medium">
            {t("emptyCopyDidNotRunTitle")}
          </h3>
          <p className="mt-1 text-xs text-muted-foreground">
            {t("emptyCopyDidNotRunBody")}
          </p>
        </div>
      )}
    ...
    <div className="flex flex-col gap-4" aria-label={t("outcomesLabel")}>
      {outcome.completed.length > 0 && (
        <section aria-labelledby="completed-title">
          ...
          <h3 id="completed-title" className="mb-2 text-sm font-medium">
            {t("copiedFiles")}
          </h3>
          <ul className="overflow-hidden rounded-xl border border-border bg-card">
      ...
      {outcome.failed.length > 0 && (
        <section aria-labelledby="failed-title">
          ...
          <h3 id="failed-title" className="mb-2 text-sm font-medium">
            {t("failedFile")}
          </h3>
          <ul className="overflow-hidden rounded-xl border border-border bg-card">
            {outcome.failed.map((row, i) => (
              ...
              >
                ...
                <span className="text-xs text-destructive">
                  {getErrorLabel(
                    locale,
                    String(row.code ?? row.errorCode ?? row.kind ?? ""),
                  )}
                  {row.partialTargetMayRemain
                    ? ` ${t("partialTargetMayRemain")}`
                    : ""}
                </span>
              </li>
            ))}
      ...
      {outcome.pending.length > 0 && (
        <section aria-labelledby="pending-title">
          ...
          <h3 id="pending-title" className="mb-2 text-sm font-medium">
            {t("pendingAfterFailure")}
          </h3>
          <ul className="overflow-hidden rounded-xl border border-border bg-card">
            {outcome.pending.map((row, i) => (
              <li
                key={i}
                className="flex items-center gap-2 border-b border-border px-4 py-3 last:border-b-0 font-mono text-sm"
              >
                <bdi>{row.sourceLabel ?? ""}</bdi>
                <span className="mx-1 text-muted-foreground">→</span>
                <bdi>{row.targetLabel ?? ""}</bdi>
