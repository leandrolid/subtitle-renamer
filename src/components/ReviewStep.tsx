import { ArrowRight, Captions, Film, CheckCircle2, AlertTriangle, XCircle, ChevronLeft } from 'lucide-react';
...
const STATUS_META: Record<RowStatus, { label: (t: (k: string) => string) => string; icon: typeof CheckCircle2; className: string }> = {
  matched: {
...
function StatusBadge({ status, t }: { status: RowStatus; t: (k: string) => string }) {
...
export function ReviewStep({ t, visibleStep, copies, skips, locale, directoryLabel, onBack, onContinue, continueDisabled, backDisabled }: Props) {
    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-6 py-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="space-y-1">
          <h2 className="text-xl font-semibold tracking-tight">{t('reviewTitle')}</h2>
          {directoryLabel && (
            <p className="font-mono text-xs text-muted-foreground">{directoryLabel}</p>
          )}
        </div>
        <div className="flex items-center gap-2 text-xs" aria-label={t('planCountsLabel')}>
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-card">
        ...
        <ul>
          {copies.map((row, i) => (
            <li key={i} className="grid grid-cols-[1fr_auto_1fr_7rem] items-center gap-4 border-b border-border px-4 py-3 last:border-b-0">
              <div className="flex min-w-0 items-center gap-2.5">
                <Captions className="size-4 shrink-0 text-brand" />
                <p className="truncate font-mono text-sm">{row.sourceLabel ?? t('unknownSubtitle')}</p>
              </div>
          ...
          {skips.map((row, i) => (
            <li key={`skip-${i}`} className="grid grid-cols-[1fr_auto_1fr_7rem] items-center gap-4 border-b border-border px-4 py-3 last:border-b-0 opacity-60">
              <div className="flex min-w-0 items-center gap-2.5">
                <Captions className="size-4 shrink-0 text-muted-foreground" />
                <p className="truncate font-mono text-sm">{row.sourceLabel ?? row.label ?? t('unknownSubtitle')}</p>
              </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <Button variant="brand" onClick={onContinue} disabled={continueDisabled} className="gap-1.5">
          {t('continue')}
